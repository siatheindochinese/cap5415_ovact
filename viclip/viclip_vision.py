...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masking_prob=0.0):
        ...
        if hasattr(self, 'temporal_positional_embedding'):
            temporal_positional_embedding = self.temporal_positional_embedding
            temporal_positional_embedding = interpolate_temporal_pos_embed(temporal_positional_embedding.unsqueeze(2), T).squeeze(2)
            if x.size(1) == 1:
                # This is a workaround for unused parameter issue
                x = x + temporal_positional_embedding.mean(1)
            else:
